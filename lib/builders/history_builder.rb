class HistoryBuilder
  ...
  def rollback_to_starting_cardset
    cardset = board.tasks
  ...
  def initial_state cardset
    card_states = {}

    cardset.each do |card_id|
        ...
        begin
          ...
          when "updated"
            ...
            when ["assigned_user_id"]
              if card_id == 11700912 && change_time < Date.new(2018,1,1)
                card_state.user_id = nil
              else
                card_state.user_id = change["data"]["values"][0].to_i
              end 
              card_state.start = change_time
            when ["block_reason"]
              if card_id == 11700912 && change_time < Date.new(2018,1,1)
              ...
              card_state.start = change_time
            when ['description'], ['name'] # don't care about these
            when ["card_type_id"] # ignore for now
  ...
  def create_activities
    @changelogs.reverse_each do |change|
      next unless change["changed_object_type"] == "Task"
      next if What_not_interested_in.include?change["what"] 
      ...
      begin
        ...
        when "archived", "unarchived" #no change
